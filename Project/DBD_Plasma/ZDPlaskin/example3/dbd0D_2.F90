...
! number of pulses
!
  integer, parameter          :: ipulses = 20
!
! filename
!
  character(*), parameter     :: file_in = 'data_in_2.dat', file_out = 'out.dat'
...
11 write(*,'(x,A,i0,A)') 'found ', idata_max, ' points in ' // file_in // ' file'
  ...
  if(idata_max .le. 1) then
    ...
    allocate(data_in(idata_max,2))
    rewind(1)
    read(1,*)
    do i = 1, idata_max
      read(1,*) data_in(i,1:2)
    enddo
    close(1)
  endif
...
!
  ...
  call ZDPlasKin_set_density('E',3.54d4) ! fixed electron density
