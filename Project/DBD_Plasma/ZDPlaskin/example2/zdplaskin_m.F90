...
! config
!
  integer, parameter :: species_max = 8, species_electrons = 1, species_length = 8, reactions_max = 13, reactions_length = 20
...
!
  ...
  integer, parameter, private               :: bolsig_species_max = 2, bolsig_species_length = 3, bolsig_rates_max = 4 
...
!
  ...
  /"E       ","AR      ","AR*     ","AR^+    ","AR2^+   ","AR(W)^+ ","AR2(W)^+","E(W)    "/
  data reaction_sign(1:reactions_max) &
  /"bolsig:AR->AR^+     ","bolsig:AR->AR*      ","bolsig:AR*->AR      ","bolsig:AR*->AR^+    ","AR2^++E=>AR*+AR     ",&
   "AR2^++AR=>AR^++AR+AR","AR*+AR*=>AR2^++E    ","AR^++E+E=>AR+E      ","AR*+AR+AR=>AR+AR+AR ","AR^++AR+AR=>AR2^++AR",&
   "AR^+=>AR(W)^+       ","AR2^+=>AR2(W)^+     ","E=>E(W)             "/
...
300 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file <" &
    ...
    close(ifile_unit)
311 format(241x,8(1x,i9))
312 format(A2,1x,A20,1x,8(1x,A9))
313 format(i2,1x,A20,1x,8(1x,1pd9.2))
  endif
  return
...
100 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
    ...
    do i = 1, species_max
      write(ifile_unit,"(13(i3))",err=200) int(mrtm(i,:))
    enddo
    lerror = .false.
200 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
...
400 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
    ...
    write(ifile_unit,"(1x,A12,13(111x,i2.2))",err=500) "Time_s", ( i, i = 1, reactions_max )
    lerror = .false.
500 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
  ...
  if( ( time - densav(0,1) ) >= rtol .or. lfirst ) then
    ...
    if( rtol > qtplaskin_rtol .or. lfirst ) then
      ...
      write(ifile_unit,"(14(1pe13.4))") densav(0,2), rrt_loc(:)
      close(ifile_unit)
      densav(:,1) = densav(:,2)
    endif
  endif
...
subroutine ZDPlasKin_reac_source_matrix(reac_rate_local,reac_source_local)
  ...
  reac_source_local(1,01) = + reac_rate_local(01) 
  reac_source_local(2,01) = - reac_rate_local(01) 
  reac_source_local(4,01) = + reac_rate_local(01) 
  reac_source_local(2,02) = - reac_rate_local(02) 
  reac_source_local(3,02) = + reac_rate_local(02) 
  reac_source_local(2,03) = + reac_rate_local(03) 
  reac_source_local(3,03) = - reac_rate_local(03) 
  reac_source_local(1,04) = + reac_rate_local(04) 
  ...
  reac_source_local(2,05) = + reac_rate_local(05) 
  reac_source_local(3,05) = + reac_rate_local(05) 
  reac_source_local(5,05) = - reac_rate_local(05) 
  reac_source_local(2,06) = + reac_rate_local(06) 
  reac_source_local(4,06) = + reac_rate_local(06) 
  reac_source_local(5,06) = - reac_rate_local(06) 
  reac_source_local(1,07) = + reac_rate_local(07) 
  reac_source_local(3,07) = - reac_rate_local(07) * 2.d0
  reac_source_local(5,07) = + reac_rate_local(07) 
  reac_source_local(1,08) = - reac_rate_local(08) 
  reac_source_local(2,08) = + reac_rate_local(08) 
  reac_source_local(4,08) = - reac_rate_local(08) 
  reac_source_local(2,09) = + reac_rate_local(09) 
  reac_source_local(3,09) = - reac_rate_local(09) 
  reac_source_local(2,10) = - reac_rate_local(10) 
  reac_source_local(4,10) = - reac_rate_local(10) 
  reac_source_local(5,10) = + reac_rate_local(10) 
  reac_source_local(4,11) = - reac_rate_local(11) 
  reac_source_local(6,11) = + reac_rate_local(11) 
  reac_source_local(5,12) = - reac_rate_local(12) 
  reac_source_local(7,12) = + reac_rate_local(12) 
  reac_source_local(1,13) = - reac_rate_local(13) 
  reac_source_local(8,13) = + reac_rate_local(13) 
  return
...
subroutine ZDPlasKin_fex(neq,t,y,ydot)
  ...
  rrt(02) = rrt(02) * density(1) * density(2) 
  rrt(03) = rrt(03) * density(1) * density(3) 
  rrt(04) = rrt(04) * density(1) * density(3) 
  rrt(05) = rrt(05) * density(1) * density(5) 
  rrt(06) = rrt(06) * density(2) * density(5) 
  rrt(07) = rrt(07) * density(3)**2 
  rrt(08) = rrt(08) * density(1)**2 * density(4) 
  rrt(09) = rrt(09) * density(2)**2 * density(3) 
  rrt(10) = rrt(10) * density(2)**2 * density(4) 
  rrt(11) = rrt(11) * density(4) 
  rrt(12) = rrt(12) * density(5) 
  rrt(13) = rrt(13) * density(1) 
  ydot(1) = +rrt(01)+rrt(04)-rrt(05)+rrt(07)-rrt(08)-rrt(13) 
  ydot(2) = -rrt(01)-rrt(02)+rrt(03)+rrt(05)+rrt(06)+rrt(08)+rrt(09)-rrt(10) 
  ydot(3) = +rrt(02)-rrt(03)-rrt(04)+rrt(05)- 2.d0 * rrt(07)-rrt(09) 
  ydot(4) = +rrt(01)+rrt(04)+rrt(06)-rrt(08)-rrt(10)-rrt(11) 
  ydot(5) = -rrt(05)-rrt(06)+rrt(07)+rrt(10)-rrt(12) 
  ydot(6) = +rrt(11) 
  ydot(7) = +rrt(12) 
  ydot(8) = +rrt(13) 
...
subroutine ZDPlasKin_jex(neq,t,y,ml,mu,pd,nrpd)
  ...
  density(:) = y(1:species_max)
  call ZDPlasKin_reac_rates(t)
  pd(1,1) = pd(1,1) + rrt(01) * density(2) 
  pd(1,2) = pd(1,2) + rrt(01) * density(1) 
  pd(2,1) = pd(2,1) - rrt(01) * density(2) 
  pd(2,2) = pd(2,2) - rrt(01) * density(1) 
  pd(4,1) = pd(4,1) + rrt(01) * density(2) 
  pd(4,2) = pd(4,2) + rrt(01) * density(1) 
  pd(2,1) = pd(2,1) - rrt(02) * density(2) 
  pd(2,2) = pd(2,2) - rrt(02) * density(1) 
  pd(3,1) = pd(3,1) + rrt(02) * density(2) 
  pd(3,2) = pd(3,2) + rrt(02) * density(1) 
  pd(2,1) = pd(2,1) + rrt(03) * density(3) 
  pd(2,3) = pd(2,3) + rrt(03) * density(1) 
  pd(3,1) = pd(3,1) - rrt(03) * density(3) 
  pd(3,3) = pd(3,3) - rrt(03) * density(1) 
  pd(1,1) = pd(1,1) + rrt(04) * density(3) 
  pd(1,3) = pd(1,3) + rrt(04) * density(1) 
  pd(3,1) = pd(3,1) - rrt(04) * density(3) 
  pd(3,3) = pd(3,3) - rrt(04) * density(1) 
  pd(4,1) = pd(4,1) + rrt(04) * density(3) 
  pd(4,3) = pd(4,3) + rrt(04) * density(1) 
  pd(1,1) = pd(1,1) - rrt(05) * density(5) 
  pd(1,5) = pd(1,5) - rrt(05) * density(1) 
  pd(2,1) = pd(2,1) + rrt(05) * density(5) 
  pd(2,5) = pd(2,5) + rrt(05) * density(1) 
  pd(3,1) = pd(3,1) + rrt(05) * density(5) 
  pd(3,5) = pd(3,5) + rrt(05) * density(1) 
  pd(5,1) = pd(5,1) - rrt(05) * density(5) 
  pd(5,5) = pd(5,5) - rrt(05) * density(1) 
  pd(2,2) = pd(2,2) + rrt(06) * density(5) 
  pd(2,5) = pd(2,5) + rrt(06) * density(2) 
  pd(4,2) = pd(4,2) + rrt(06) * density(5) 
  pd(4,5) = pd(4,5) + rrt(06) * density(2) 
  pd(5,2) = pd(5,2) - rrt(06) * density(5) 
  pd(5,5) = pd(5,5) - rrt(06) * density(2) 
  pd(1,3) = pd(1,3) + rrt(07) * density(3) * 2.0d0
  pd(3,3) = pd(3,3) - rrt(07) * density(3) * 4.0d0
  pd(5,3) = pd(5,3) + rrt(07) * density(3) * 2.0d0
  pd(1,1) = pd(1,1) - rrt(08) * density(1) * density(4) * 2.0d0
  pd(1,4) = pd(1,4) - rrt(08) * density(1)**2 
  pd(2,1) = pd(2,1) + rrt(08) * density(1) * density(4) * 2.0d0
  pd(2,4) = pd(2,4) + rrt(08) * density(1)**2 
  pd(4,1) = pd(4,1) - rrt(08) * density(1) * density(4) * 2.0d0
  pd(4,4) = pd(4,4) - rrt(08) * density(1)**2 
  pd(2,2) = pd(2,2) + rrt(09) * density(2) * density(3) * 2.0d0
  pd(2,3) = pd(2,3) + rrt(09) * density(2)**2 
  pd(3,2) = pd(3,2) - rrt(09) * density(2) * density(3) * 2.0d0
  pd(3,3) = pd(3,3) - rrt(09) * density(2)**2 
  pd(2,2) = pd(2,2) - rrt(10) * density(2) * density(4) * 2.0d0
  pd(2,4) = pd(2,4) - rrt(10) * density(2)**2 
  pd(4,2) = pd(4,2) - rrt(10) * density(2) * density(4) * 2.0d0
  pd(4,4) = pd(4,4) - rrt(10) * density(2)**2 
  pd(5,2) = pd(5,2) + rrt(10) * density(2) * density(4) * 2.0d0
  pd(5,4) = pd(5,4) + rrt(10) * density(2)**2 
  pd(4,4) = pd(4,4) - rrt(11) 
  pd(6,4) = pd(6,4) + rrt(11) 
  pd(5,5) = pd(5,5) - rrt(12) 
  pd(7,5) = pd(7,5) + rrt(12) 
  pd(1,1) = pd(1,1) - rrt(13) 
  pd(8,1) = pd(8,1) + rrt(13) 
...
subroutine ZDPlasKin_reac_rates(Time)
  ...
  * ((2.405D0/RADIUS)**2 + (3.141/GAP_LENGTH)**2)
  ...
  rrt(05) = 8.5D-7*(TE/300.0D0)**(-0.67D0)
  rrt(06) = 6.06D-6/TGAS*EXP(-15130.0D0/TGAS)
  rrt(07) = 6.0D-10
  rrt(08) = 8.75D-27*(TE/11600.0D0)**(-4.5D0)
  rrt(09) = 1.4D-32
  rrt(10) = 2.25D-31*(TGAS/300.0D0)**(-0.4D0)
  rrt(11) = DIFF_RATE
  rrt(12) = DIFF_RATE
  rrt(13) = DIFF_RATE
