...
def compile_zdp(name):  # ZDPlaskin 컴파일 함수 정의
    compile_command = [  # 컴파일 명령어 리스트 생성
        'gfortran', '-o', name, 'dvode_f90_m.F90', 'zdplaskin_m.F90', 'run_12-5kV.F90', 'bolsig_x86_64_g.dll'
    ]
...
# 그래프 데이터를 CSV 파일로 저장
def save_graph_data_to_csv(exp, sim, residence_times, exp_list):
    # 데이터 준비
    data = {'Residence Time (s)': residence_times}
    
    # 각 물질별 실험값과 시뮬레이션값 추가
    for i, species in enumerate(exp_list):
        # 실험값 추출
        exp_values = [exp_data[i] for exp_data in exp]
        # 시뮬레이션값 추출
        ...
        # 데이터프레임에 추가
        data[f'{species}_Exp'] = exp_values
        data[f'{species}_Sim'] = sim_values
    
    # 데이터프레임 생성 및 CSV 저장
    df_graph = pd.DataFrame(data)
    df_graph.to_csv('graph_data.csv', index=False)
    print('그래프 데이터가 graph_data.csv 파일에 저장되었습니다.')

# 그래프 그리기 전에 데이터 저장
save_graph_data_to_csv(exp, sim, residence_times, exp_list)
