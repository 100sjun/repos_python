...
!
  ...
  double precision, parameter :: gas_pressure      = 1.0d0,    & ! pressure, atm
                                 gas_temperature   = 273.15d0,   & ! gas temperature, K
                                 ini_elec_density  = 1.0d9,    & ! initial electron density, cm-3
                                 ini_gas_density   = 101325.0d0 * gas_pressure & 
                                                   / gas_temperature / 1.38d-17,  & ! gas density, cm-3
                                 applied_voltage   = 1.5d4,    & ! applied voltage, V
                                 gap_distance      = 6.0d-1 ! gap_distance, cm
  double precision            :: time, time_end, dtime, EN,& ! times, s // Reduced Electric field, Td
                                 t1, t2, tc, all_neutral                       ! calculation time, mobility
  integer                     :: i
!
! print
!
...
! initialization of variables
    ...
    time_end = 5.66d0
    dtime = 1e-4
    EN = applied_voltage/gap_distance/gas_pressure*gas_temperature*(1.380649d-23)*1e6/(1.0d-17)
...
!
  ...
  do while(time .lt. time_end)
    ...
    EN = applied_voltage/gap_distance/all_neutral/(1.0d-17)
