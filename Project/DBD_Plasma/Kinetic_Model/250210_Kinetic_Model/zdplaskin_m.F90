...
! config
!
  integer, parameter :: species_max = 8, species_electrons = 1, species_length = 8, reactions_max = 12, reactions_length = 22
...
!
  ...
  integer, parameter, private               :: bolsig_species_max = 4, bolsig_species_length = 8, bolsig_rates_max = 11 
...
!
  ...
  /"bolsig:CH4->CH4(V24)  ","bolsig:CH4->CH4(V13)  ","bolsig:CH4(V24)->CH4  ","bolsig:CH4(V24)->CH4  ","bolsig:CH4->CH4^+     ",&
   "bolsig:CH4(V24)->CH4^+","bolsig:CH4(V13)->CH4^+","bolsig:CH4^+->CH4     ","bolsig:CH4^+->CH4(V24)","bolsig:CH4^+->CH4(V13)",&
   "bolsig:CH4->CH3H      ","H+H=>H2               "/
...
300 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file <" &
    ...
    close(ifile_unit)
311 format(261x,8(1x,i9))
312 format(A2,1x,A22,1x,8(1x,A9))
313 format(i2,1x,A22,1x,8(1x,1pd9.2))
  endif
  return
...
100 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
    ...
    do i = 1, species_max
      write(ifile_unit,"(12(i3))",err=200) int(mrtm(i,:))
    enddo
    lerror = .false.
200 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
...
400 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
    ...
    write(ifile_unit,"(1x,A12,12(111x,i2.2))",err=500) "Time_s", ( i, i = 1, reactions_max )
    lerror = .false.
500 if( lerror ) call ZDPlasKin_stop("ZDPlasKin ERROR: cannot write to file " // &
  ...
  if( ( time - densav(0,1) ) >= rtol .or. lfirst ) then
    ...
    if( rtol > qtplaskin_rtol .or. lfirst ) then
      ...
      write(ifile_unit,"(13(1pe13.4))") densav(0,2), rrt_loc(:)
      close(ifile_unit)
      densav(:,1) = densav(:,2)
    endif
  endif
...
subroutine ZDPlasKin_reac_source_matrix(reac_rate_local,reac_source_local)
  ...
  reac_source_local(3,01) = - reac_rate_local(01) 
  reac_source_local(5,01) = + reac_rate_local(01) 
  reac_source_local(3,02) = - reac_rate_local(02) 
  reac_source_local(4,02) = + reac_rate_local(02) 
  reac_source_local(3,03) = + reac_rate_local(03) 
  reac_source_local(5,03) = - reac_rate_local(03) 
  reac_source_local(3,04) = + reac_rate_local(04) 
  reac_source_local(4,04) = - reac_rate_local(04) 
  reac_source_local(1,05) = + reac_rate_local(05) 
  reac_source_local(3,05) = - reac_rate_local(05) 
  reac_source_local(6,05) = + reac_rate_local(05) 
  reac_source_local(1,06) = + reac_rate_local(06) 
  reac_source_local(5,06) = - reac_rate_local(06) 
  reac_source_local(6,06) = + reac_rate_local(06) 
  reac_source_local(1,07) = + reac_rate_local(07) 
  reac_source_local(4,07) = - reac_rate_local(07) 
  reac_source_local(6,07) = + reac_rate_local(07) 
  reac_source_local(1,08) = - reac_rate_local(08) 
  reac_source_local(3,08) = + reac_rate_local(08) 
  reac_source_local(6,08) = - reac_rate_local(08) 
  reac_source_local(1,09) = - reac_rate_local(09) 
  reac_source_local(5,09) = + reac_rate_local(09) 
  reac_source_local(6,09) = - reac_rate_local(09) 
  reac_source_local(1,10) = - reac_rate_local(10) 
  reac_source_local(4,10) = + reac_rate_local(10) 
  reac_source_local(6,10) = - reac_rate_local(10) 
  reac_source_local(2,11) = + reac_rate_local(11) 
  reac_source_local(3,11) = - reac_rate_local(11) 
  reac_source_local(7,11) = + reac_rate_local(11) 
  reac_source_local(7,12) = - reac_rate_local(12) * 2.d0
  reac_source_local(8,12) = + reac_rate_local(12) 
  return
...
subroutine ZDPlasKin_fex(neq,t,y,ydot)
  ...
  rrt(01) = rrt(01) * density(1) * density(3) 
  rrt(02) = rrt(02) * density(1) * density(3) 
  rrt(03) = rrt(03) * density(1) * density(5) 
  rrt(04) = rrt(04) * density(1) * density(4) 
  rrt(05) = rrt(05) * density(1) * density(3) 
  rrt(06) = rrt(06) * density(1) * density(5) 
  rrt(07) = rrt(07) * density(1) * density(4) 
  rrt(08) = rrt(08) * density(1)**2 * density(6) 
  rrt(09) = rrt(09) * density(1)**2 * density(6) 
  rrt(10) = rrt(10) * density(1)**2 * density(6) 
  rrt(11) = rrt(11) * density(1) * density(3) 
  rrt(12) = rrt(12) * density(7)**2 
  ydot(1) = +rrt(05)+rrt(06)+rrt(07)-rrt(08)-rrt(09)-rrt(10) 
  ydot(2) = +rrt(11) 
  ydot(3) = -rrt(01)-rrt(02)+rrt(03)+rrt(04)-rrt(05)+rrt(08)-rrt(11) 
  ydot(4) = +rrt(02)-rrt(04)-rrt(07)+rrt(10) 
  ydot(5) = +rrt(01)-rrt(03)-rrt(06)+rrt(09) 
  ydot(6) = +rrt(05)+rrt(06)+rrt(07)-rrt(08)-rrt(09)-rrt(10) 
  ydot(7) = +rrt(11)- 2.d0 * rrt(12) 
  ydot(8) = +rrt(12) 
...
subroutine ZDPlasKin_jex(neq,t,y,ml,mu,pd,nrpd)
  ...
  density(:) = y(1:species_max)
  call ZDPlasKin_reac_rates(t)
  pd(3,1) = pd(3,1) - rrt(01) * density(3) 
  pd(3,3) = pd(3,3) - rrt(01) * density(1) 
  pd(5,1) = pd(5,1) + rrt(01) * density(3) 
  pd(5,3) = pd(5,3) + rrt(01) * density(1) 
  pd(3,1) = pd(3,1) - rrt(02) * density(3) 
  pd(3,3) = pd(3,3) - rrt(02) * density(1) 
  pd(4,1) = pd(4,1) + rrt(02) * density(3) 
  pd(4,3) = pd(4,3) + rrt(02) * density(1) 
  pd(3,1) = pd(3,1) + rrt(03) * density(5) 
  pd(3,5) = pd(3,5) + rrt(03) * density(1) 
  pd(5,1) = pd(5,1) - rrt(03) * density(5) 
  pd(5,5) = pd(5,5) - rrt(03) * density(1) 
  pd(3,1) = pd(3,1) + rrt(04) * density(4) 
  pd(3,4) = pd(3,4) + rrt(04) * density(1) 
  pd(4,1) = pd(4,1) - rrt(04) * density(4) 
  pd(4,4) = pd(4,4) - rrt(04) * density(1) 
  pd(1,1) = pd(1,1) + rrt(05) * density(3) 
  pd(1,3) = pd(1,3) + rrt(05) * density(1) 
  pd(3,1) = pd(3,1) - rrt(05) * density(3) 
  pd(3,3) = pd(3,3) - rrt(05) * density(1) 
  pd(6,1) = pd(6,1) + rrt(05) * density(3) 
  pd(6,3) = pd(6,3) + rrt(05) * density(1) 
  pd(1,1) = pd(1,1) + rrt(06) * density(5) 
  pd(1,5) = pd(1,5) + rrt(06) * density(1) 
  pd(5,1) = pd(5,1) - rrt(06) * density(5) 
  pd(5,5) = pd(5,5) - rrt(06) * density(1) 
  pd(6,1) = pd(6,1) + rrt(06) * density(5) 
  pd(6,5) = pd(6,5) + rrt(06) * density(1) 
  pd(1,1) = pd(1,1) + rrt(07) * density(4) 
  pd(1,4) = pd(1,4) + rrt(07) * density(1) 
  pd(4,1) = pd(4,1) - rrt(07) * density(4) 
  pd(4,4) = pd(4,4) - rrt(07) * density(1) 
  pd(6,1) = pd(6,1) + rrt(07) * density(4) 
  pd(6,4) = pd(6,4) + rrt(07) * density(1) 
  pd(1,1) = pd(1,1) - rrt(08) * density(1) * density(6) * 2.0d0
  pd(1,6) = pd(1,6) - rrt(08) * density(1)**2 
  pd(3,1) = pd(3,1) + rrt(08) * density(1) * density(6) * 2.0d0
  pd(3,6) = pd(3,6) + rrt(08) * density(1)**2 
  pd(6,1) = pd(6,1) - rrt(08) * density(1) * density(6) * 2.0d0
  pd(6,6) = pd(6,6) - rrt(08) * density(1)**2 
  pd(1,1) = pd(1,1) - rrt(09) * density(1) * density(6) * 2.0d0
  pd(1,6) = pd(1,6) - rrt(09) * density(1)**2 
  pd(5,1) = pd(5,1) + rrt(09) * density(1) * density(6) * 2.0d0
  pd(5,6) = pd(5,6) + rrt(09) * density(1)**2 
  pd(6,1) = pd(6,1) - rrt(09) * density(1) * density(6) * 2.0d0
  pd(6,6) = pd(6,6) - rrt(09) * density(1)**2 
  pd(1,1) = pd(1,1) - rrt(10) * density(1) * density(6) * 2.0d0
  pd(1,6) = pd(1,6) - rrt(10) * density(1)**2 
  pd(4,1) = pd(4,1) + rrt(10) * density(1) * density(6) * 2.0d0
  pd(4,6) = pd(4,6) + rrt(10) * density(1)**2 
  pd(6,1) = pd(6,1) - rrt(10) * density(1) * density(6) * 2.0d0
  pd(6,6) = pd(6,6) - rrt(10) * density(1)**2 
  pd(2,1) = pd(2,1) + rrt(11) * density(3) 
  pd(2,3) = pd(2,3) + rrt(11) * density(1) 
  pd(3,1) = pd(3,1) - rrt(11) * density(3) 
  pd(3,3) = pd(3,3) - rrt(11) * density(1) 
  pd(7,1) = pd(7,1) + rrt(11) * density(3) 
  pd(7,3) = pd(7,3) + rrt(11) * density(1) 
  pd(7,7) = pd(7,7) - rrt(12) * density(7) * 4.0d0
  pd(8,7) = pd(8,7) + rrt(12) * density(7) * 2.0d0
...
subroutine ZDPlasKin_reac_rates(Time)
  ...
  DOUBLE PRECISION, PARAMETER :: F2 = 1.0000D3
  DOUBLE PRECISION, PARAMETER :: F3 = 1.0000D0
  DOUBLE PRECISION, PARAMETER :: F4 = 1.0000D16
  call ZDPlasKin_bolsig_rates()
  ...
  rrt(08) = F2*bolsig_rates(bolsig_pointer(8))
  rrt(09) = F2*bolsig_rates(bolsig_pointer(9))
  rrt(10) = F2*bolsig_rates(bolsig_pointer(10))
  rrt(11) = F3*bolsig_rates(bolsig_pointer(11))
  rrt(12) = F4*5.52D-30*TGAS**(-1.00)
