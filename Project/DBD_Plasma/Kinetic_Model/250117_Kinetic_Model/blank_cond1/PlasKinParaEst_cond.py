...
X_data = pd.read_csv('X_final.csv', header= None)
y_data = pd.read_csv('y_final.csv', header= None)
pre_X = np.array([np.array(X_data.iloc[i]) for i in range(len(X_data))])
pre_Y = np.array([np.array(y_data.iloc[i]) for i in range(len(y_data))])
