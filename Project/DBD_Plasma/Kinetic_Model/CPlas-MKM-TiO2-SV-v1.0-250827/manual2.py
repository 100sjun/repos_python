...
# 각 체류시간별 결과 비교
residence_times = [4.295445815, 5.522716048, 7.731802468, 12.88633745]
...
for i in range(len(rx)):
    ...
    # 반응 번호를 포함한 고유한 키 생성
    unique_key = f"{rx_numbers[i]}_{rx[i]}"
    reaction_extents[unique_key] = reaction

df_rex = pd.DataFrame(reaction_extents)
rx_end = df_rex.iloc[-1].sort_values(ascending=False)

# 모든 반응의 반응식과 번호를 내림차순(전체)으로 출력
print("모든 반응식 (번호 포함, 내림차순):")
for reaction_name, value in rx_end.sort_values(ascending=False).items():
    if reaction_name == 'Time [s]':
        continue
    # 고유 키에서 반응 번호와 반응식 분리
    if '_' in reaction_name:
        rx_num, rx_formula = reaction_name.split('_', 1)
        print(f"{rx_num}: {rx_formula}  {value}")
    else:
        print(f"{reaction_name}  {value}")

# 모든 종의 농도를 내림차순(전체)으로 출력
print("\n모든 종 농도 (내림차순):")
print(df_sp.iloc[-1].sort_values(ascending=False))
