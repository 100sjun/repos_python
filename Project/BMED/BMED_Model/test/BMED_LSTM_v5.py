...
class BMEDDataset(Dataset):
    ...
    def __init__(self, data_path, train=True, train_ratio=0.8, scalers=None):
        self.train = train
    ...
    def prepare_data(self):
        ...
        for exp_id in self.exps_to_use:
            ...
            for i in range(len(exp_data) - 1):
                ...
                mol_changes.append(mol_change)
                states.append(state)

                # save index
                exp_indices.append(total_samples)
                total_samples += 1
            
            # save index of current experiment
            if exp_indices:
                self.exp_indices.append((exp_id, exp_indices))

        # Numpy 배열로 변환
        self.features_raw = np.array(features, dtype=np.float32)
        self.mol_changes_raw = np.array(mol_changes, dtype=np.float32)
        self.states_raw = np.array(states, dtype=np.float32)

        # Data normalization
        if self.train:
            self.feature_scaler.fit(self.features_raw)
            self.mol_change_scaler.fit(self.mol_changes_raw)
            self.state_scaler.fit(self.states_raw)

        # Normalization
        self.features = self.feature_scaler.transform(self.features_raw)
        self.mol_changes = self.mol_change_scaler.transform(self.mol_changes_raw)
        self.states = self.state_scaler.transform(self.states_raw)

        print(f"Data loaded: {len(self.features)} samples")
        print(f"Features shape: {self.features.shape}")
        print(f"Mol changes shape: {self.mol_changes.shape}")
    ...
    def __getitem__(self, idx):
        feature = self.features[idx].reshape(1, -1)  # [1, 10] 형태 (seq_len=1, features=10)
        mol_change = self.mol_changes[idx].reshape(1, -1)  # [1, 4] 형태
        state = self.states[idx].reshape(1, -1)  # [1, 10] 형태
        
        return (
            torch.FloatTensor(feature),        # [1, 10] 형태
            torch.FloatTensor(mol_change),     # [1, 4] 형태  
            torch.FloatTensor(state)           # [1, 10] 형태
        )
...
class MembraneSystemModel(nn.Module):
    ...
    def __init__(self, lstm_units=64, lstm_layer=2, fc_units=32, fc_layer=3,time_step=0.1):
    ...
    def forward(self, x, hidden=None):
        '''LSTM 처리'''
        # x는 [batch_size, seq_len=1, feature_dim=10] 형태
        lstm_out, hidden = self.lstm(x, hidden)  # lstm_out: [batch_size, seq_len=1, lstm_units]
        
        '''몰 변화량 예측'''
        # 시퀀스 차원 제거하여 FC 레이어에 전달
        mol_changes = self.fc(lstm_out.squeeze(1))  # [batch_size, 4] 형태
        
        '''물리 법칙 적용'''
        # 현재 상태도 시퀀스 차원 제거
        new_states = self.physics_layer(mol_changes, x.squeeze(1))  # [batch_size, 10] 형태
        
        # 일관성을 위해 몰 변화량과 상태 모두 2D로 반환
        return mol_changes, new_states, hidden
...
class MembraneSystemTrainer:
    ...
    def train(self, train_loader, val_loader=None, patience=100):
        """
        모델 훈련 - 실험 단위로 처리하되 배치 크기 문제 해결
        
        입력:
        - train_loader: 훈련 데이터 로더 (ExperimentBatchSampler 사용)
        - val_loader: 검증 데이터 로더 (옵션)
        - patience: early stopping을 위한 인내심
        """
        self.model.train()
        train_losses = []
        val_losses = []
        ...
        for epoch in range(self.epochs):
            ...
            # 각 실험을 순차적으로 처리하기 위한 데이터 그룹화
            exp_batches = {}
            current_exp_id = None
            
            # 현재 데이터로더에서 실험별로 배치를 그룹화
            for batch_idx, (features, mol_change_targets, state_targets) in enumerate(train_loader):
                # batch_sampler가 get_current_batch_indices 메서드를 가지고 있으면 사용
                if hasattr(train_loader.batch_sampler, 'get_current_batch_indices'):
                    try:
                        indices = train_loader.batch_sampler.get_current_batch_indices()
                        # 이 배치의 첫 번째 인덱스가 속하는 실험 찾기
                        for exp_id, exp_indices in train_loader.dataset.exp_indices:
                            if indices[0] in exp_indices:
                                if exp_id not in exp_batches:
                                    exp_batches[exp_id] = []
                                exp_batches[exp_id].append((features, mol_change_targets, state_targets))
                                break
                    except (AttributeError, IndexError):
                        # fallback: 각 배치를 개별 실험으로 처리
                        exp_id = f"exp_{batch_idx}"
                        if exp_id not in exp_batches:
                            exp_batches[exp_id] = []
                        exp_batches[exp_id].append((features, mol_change_targets, state_targets))
                else:
                    # batch_sampler가 메서드를 가지고 있지 않으면 각 배치를 개별 실험으로 처리
                    exp_id = f"exp_{batch_idx}"
                    if exp_id not in exp_batches:
                        exp_batches[exp_id] = []
                    exp_batches[exp_id].append((features, mol_change_targets, state_targets))
            ...
            # 각 실험별로 처리
            for exp_id, batches in exp_batches.items():
                # 각 실험의 시작에서 hidden 상태 초기화
                hidden = None
                
                # 실험 내의 모든 배치를 순서대로 처리
                for features, mol_change_targets, state_targets in batches:
                    # 데이터를 디바이스로 이동
                    ...
                    # 중요: forward pass 호출 시 hidden 상태가 None이 아니면
                    # batch_size가 일치하도록 해야 함
                    if hidden is not None:
                        # hidden이 tuple인 경우 (LSTM)
                        if isinstance(hidden, tuple):
                            # hidden과 features의 배치 크기가 다르면 hidden 초기화
                            if hidden[0].size(1) != features.size(0):
                                hidden = None
                        # hidden이 tensor인 경우 (GRU 등)
                        else:
                            if hidden.size(1) != features.size(0):
                                hidden = None
                    
                    # 모델에 features와 hidden 상태 전달
                    mol_changes, state_predictions, hidden = self.model(features, hidden)
                    
                    # hidden 상태의 그래디언트 연결 끊기
                    if hidden is not None:
                        if isinstance(hidden, tuple):
            ...
            if val_loader is not None:
                ...
                with torch.no_grad():
                    ...
                    # 검증 데이터로더에서 실험별로 배치를 그룹화
                    for batch_idx, (features, mol_change_targets, state_targets) in enumerate(val_loader):
                        # 각 배치를 개별 실험으로 처리
                        exp_id = f"val_exp_{batch_idx}"
                        if exp_id not in val_exp_batches:
                            val_exp_batches[exp_id] = []
                        val_exp_batches[exp_id].append((features, mol_change_targets, state_targets))
                    
                    # 각 실험별로 처리
                    for exp_id, batches in val_exp_batches.items():
                        # 각 실험의 시작에서 hidden 상태 초기화
                        hidden = None
                        ...
                        for features, mol_change_targets, state_targets in batches:
                            ...
                            # 모델에 features와 hidden 상태 전달
                            _, state_predictions, hidden = self.model(features, hidden)
...
def train_model_from_csv(data_path, lstm_units=64, lstm_layer=2, fc_units=32, fc_layer=3, time_step=0.1, 
    ...
    train_loader, test_loader = data_loaders_from_csv(
        data_path, batch_size, train_ratio
    )
    ...
    train_losses, val_losses = trainer.train(
        train_loader, 
        test_loader,
        patience=100  # Early stopping 적용
    )
    ...
    print(f"\n테스트 세트 Parity Plot과 모델이 '{save_dir}' 폴더에 저장되었습니다.")
...
def simulate_with_model(model, initial_state, steps=50, scalers=None, time_step=0.1):
    ...
    with torch.no_grad():
        for step in range(steps):
            ...
            # 다음 상태를 입력으로 설정 (batch_size=1, seq_len=1 유지)
