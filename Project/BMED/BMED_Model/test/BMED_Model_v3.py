...
for epoch in range(num_epochs):
    ...
    # Forward pass
    outputs, cal_times, flux = model(X_batch, S_batch, dt, max_time)
...
model.eval()
with torch.no_grad():
    predictions, cal_times, fluxes = model(X_batch, S_batch, dt, max_time)
    
    # Convert tensors to numpy arrays
    cal_times_np = cal_times.numpy()
    predictions_np = predictions[0].numpy()  # [num_steps, s_size]
    fluxes_np = torch.cat(fluxes, dim=1)[0].numpy()  # [num_steps, s_size]
