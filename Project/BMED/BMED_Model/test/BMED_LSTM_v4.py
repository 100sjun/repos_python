...
class MembraneSystemTrainer:
    ...
    def train(self, train_loader, val_loader=None, patience=10):
        """
        모델 훈련
        ...
        - patience: early stopping을 위한 인내심 (몇 에폭 동안 성능 향상이 없을 때 중단할지)
        """
        self.model.train()
        train_losses = []
        val_losses = []
...
if __name__ == "__main__":
    ...
    model, trainer, train_losses, val_losses, eval_results = train_model_from_csv(
        ...
        fc_units=64,
        fc_layer=5,
        sequence_length=1,  # 이전 3개 시점 데이터 사용
        epochs = 50,
        time_step=0.1,
        batch_size=2,
        train_ratio=0.8,
